...
def _socks_target_address(target_host: str) -> tuple[bytes, bytes]:
    """Закодировать адрес цели для SOCKS5-запроса CONNECT (RFC 1928).

    Возвращает (atyp, addr_bytes):
      - IPv4  -> (b"\\x01", 4 байта) — ATYP 0x01, адрес в бинарном виде;
      - IPv6  -> (b"\\x04", 16 байт) — ATYP 0x04, адрес в бинарном виде;
      - домен -> (b"\\x03", b"имя") — ATYP 0x03, IDNA-кодирование.

    Раньше сюда попадал старый код с target_host.encode("idna"), который
    падал с UnicodeError("label empty or too long") на адресах с пустыми
    метками (например ".1.1.1.1", "a..b") и на IPv6-адресах, а IP всегда
    отправлялся как ATYP 0x03 (DOMAINNAME) — часть SOCKS5-серверов такое
    не разбирает. Теперь IP-адреса идут нативными ATYP 0x01/0x04, домены —
    с IDNA и fallback'ом на UTF-8 при экзотических именах.
    """
    host = (target_host or "").strip()
    # IPv6 в скобках ([::1]) — чистим.
    ...
    # Доменное имя: IDNA-кодирование, при сбое — как есть (UTF-8/ASCII).
    try:
        name_bytes = host.encode("idna")
    except UnicodeError:
        name_bytes = host.encode("utf-8")
    if not name_bytes:
        return b"\x03", b""
    return b"\x03", name_bytes
...
def _socks_open_connection(
    ...
    try:
        ...
        if _recv_exact(sock, 2) != b"\x05\x00":
            sock.close()
            return None
        atyp, addr_bytes = _socks_target_address(target_host)
        if atyp == b"\x03" and not 0 < len(addr_bytes) <= 255:
            sock.close()
            return None
        request = (
            b"\x05\x01\x00"
            + atyp
            + (bytes([len(addr_bytes)]) if atyp == b"\x03" else b"")
            + addr_bytes
...
def run_with_node(
    ...
    *,
    ...
    if node is None:
        return None
    # Модуль лежит в python/checkers/ — корень репо на два уровня выше.
    ...
    runtime = XrayCoreRuntime(
        ...
        # Одноразовый runtime на ОДНУ проверку узла: без atexit-регистрации
        # (246+ экземпляров на прогон удерживали по 20-30 МБ кешей = OOM-краш),
        # без загрузки кешей результатов, без PowerShell-очистки stale-ядер
        # и без чтения ОБЩЕГО pid-файла (мог убить чужой/свой процесс).
        # Инцидент 2026-09-16: краш на середине telegram-pro fallback.
        lightweight=True,
    )
