...
Модули по слоям (снизу вверх):
  ...
  collect          — сбор узлов из подписок (+ прямые конфиги в sources.txt);
  lifecycle        — примесь: старт/стоп/pid-файлы долгоживущего ядра;
  probing          — примесь: пробы узлов (_probe_node/_probe_node_ping);
  sorting          — примесь: quick_sort_by_ping + ключи сортировки;
  stress           — примесь: stress_test/with_node_process;
  results          — примесь: выбор узла/снимок/персистентность;
  core             — сборка XrayCoreRuntime(примеси) + refresh().
