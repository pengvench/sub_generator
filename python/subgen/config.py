...
def _app_root() -> Path:
    ...
    Приоритет:
      1. Собранный PyInstaller-.exe — каталог рядом с exe
         (там лежат data/, bin/).
      2. Исходники — корень репозитория (каталог, где лежат python/, bin/,
         generate.py). Файл лежит в python/subgen/config.py, поэтому
         поднимаемся на три уровня: python/subgen -> python -> корень.
    """
