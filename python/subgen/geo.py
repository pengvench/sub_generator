...
HS_NAME_SUFFIX = "⚡"
# Telegram-медиа фильтр (t.me/s/) — ОБЯЗАТЕЛЬНЫЙ для всех узлов (v7):
# отдельной метки в имени НЕ имеет — категорий/подсветок больше нет.
...
def _geoip_fetch(ip: str, timeout: float) -> tuple[int, bytes, str]:
    """Скачать https://api.ip.sb/geoip/<ip> мимо системного hosts.

    Возвращает (status, body, error). Основной путь — hostres.direct_https_get
    (DoH-резолв + IP+SNI); при недоступности DoH деградирует до urlopen.
    """
    ...
    if response.ok and response.status == 200 and response.body:
        return response.status, response.body, response.error
    # Деградация (DoH недоступен) — обычный urlopen, hosts может влиять.
    request = Request(
        url,
        headers={
...
def serialize_working(
    ...
    *,
    ...
    Приоритет страны (v8): ИИ-гео слепок (item.ai_geo_country, ISO-код из
    CF trace — как видит exit-IP OpenAI/Anthropic) -> pyip-хинт ->
    реальный egress-IP -> geoip по host. Слепок обязателен и ставится
    раньше pyip-хинта: это то, что реально видят ИИ-сервисы.
    """
    total = len(items)
    rows: list[dict[str, Any]] = []
    for index, item in enumerate(items, start=1):
        node = item.node
        # v8: страна из ИИ-гео слепка — приоритетный источник флага.
        ai_country = str(getattr(item, "ai_geo_country", "") or "").strip().upper()
        if len(ai_country) == 2 and ai_country.isalpha():
            code, flag = ai_country, country_flag(ai_country)
        else:
        ...
        # например "🇩🇪 DE peppo ⚡". Telegram-медиа — обязательный фильтр
        # (все узлы финального списка его уже прошли), метка не ставится.
