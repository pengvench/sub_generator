...
class PipelineOptions:
    ...
    # v8: Zapret-suite — НЕОТЪЕМЛЕМАЯ часть DPI-проверки (один core-процесс
    # на узел, один этап конвейера): отдельного поля/тумблера больше нет.
    # ИИ-гео слепок — ОБЯЗАТЕЛЬНЫЙ этап (флаг страны по CF-слепку OpenAI):
...
def build_pipeline_args(options: PipelineOptions, sources: list[str]) -> list[str]:
    ...
    if options.dpi_check:
        ...
        # Zapret-suite НЕ передаётся: он всегда часть --dpi-check (v8).
    ...
    # ИИ-гео слепок — обязательный этап (не передаём --ai-check);
    # единственная опция — strict (исключить слепок-РФ).
    if options.ai_strict:
